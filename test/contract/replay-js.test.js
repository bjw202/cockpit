...
// (v2) 방 하나 판 — 대본의 걸음은 전부 본방이고, 봇을 부르는 글은 @TO 를 적는다 (ADR-015 · 018)
...
// (v2) 옛 이름 `걸음 셋짜리 대본: exit 0 · 기록 JSONL 에 bot.message_id 가 셋` 을 대체한다
test('걸음 셋짜리 대본(방 하나): exit 0 · 기록 JSONL 에 bot.message_id 가 셋', { skip }, async t => {
  const { w, tokens, main } = await cockpit(t, b => `받았습니다 #${b.messageId}`);
  ...
  const { file, record } = scriptFile(w, 'r3', {
    ...
    steps: [
      { id: 's1', room: '본방', author: 'PL', text: `${BOT} 안녕하세요`, wait: 'bot', timeout_s: 20 },
      { id: 's2', room: '본방', author: 'member', text: `${BOT} 자료입니다`, attach: ['data/yield.csv'], wait: 'bot', timeout_s: 20 },
      { id: 's3', room: '본방', author: 'member', text: `${BOT} 확인 부탁드립니다`, wait: 'bot', timeout_s: 20 },
    ],
  });
  const { out } = await replay(w, tokens, file, record);   // exit 0 이 아니면 여기서 던진다
  assert.match(out, new RegExp(`방 본방=${main.id}`));
  ...
  for (const x of r.filter(y => y.id)) {
    ...
    assert.equal(botMsg.room_id, main.id, `${x.id}: 봇이 받은 방에 답했다`);
    assert.equal(botMsg.body, `받았습니다 #${x.message_id}`);
  }
});

test('걸음 셋 — wait bot · expect then/else · 상한 1초 무응답 이 기록 JSONL 로 남는다', { skip }, async t => {
  const { w, tokens, main } = await cockpit(t, b => (b.content.includes('자료입니다') ? '읽었습니다. 맞으면 확정이라고 답해 주세요' : null));
  const { file, record } = scriptFile(w, 'r', {
    name: 'R-시험', project: '시험', actors: { PL: '김피엘', member: '김과제' }, bot: 'prodev-시험-bot',
    steps: [
      { id: 's1', room: '본방', author: 'member', text: `${BOT} 자료입니다`, wait: 'bot', timeout_s: 20,
        expect: '확정',
        then: [{ id: 's1a', room: '본방', author: 'member', text: '확정', wait: 'none' }],
        else: [{ id: 's1b', room: '본방', author: 'member', text: '여기는 안 와야 한다', wait: 'none' }] },
  ...
  assert.equal(line('s1').room_id, main.id);
...
test('expect 가 안 걸리면 else 를 돈다', { skip }, async t => {
  ...
  const { file, record } = scriptFile(w, 'e', {
    name: 'else', project: '시험', steps: [
      { id: 's1', room: '본방', author: 'member', text: `${BOT} 자료`, wait: 'bot', timeout_s: 20,
        expect: '확정',
        then: [{ id: 't1', room: '본방', author: 'member', text: 'then', wait: 'none' }],
        else: [{ id: 'e1', room: '본방', author: 'member', text: 'else', wait: 'none' }] },
    ],
  });
  await replay(w, tokens, file, record);
  const r = lines(record);
...
// (v2) 옛 이름 `첨부가 이름 그대로 올라간다` 를 대체한다
test('첨부가 이름 그대로 올라간다(본방)', { skip }, async t => {
  const { w, tokens, main } = await cockpit(t, () => null);
  fs.mkdirSync(path.join(w.dir, 'data'));
  fs.writeFileSync(path.join(w.dir, 'data', 'yield.csv'), 'a,b\n1,2\n');
  const { file, record } = scriptFile(w, 'a', {
    name: '첨부', project: '시험', steps: [
      { id: 's1', room: '본방', author: 'member', text: '자료', attach: ['data/yield.csv'], wait: 'none' },
    ],
  });
  await replay(w, tokens, file, record);
  const [msg] = w.chatDb.messagesAfter(main.id, 0);
