...
flowchart LR
    ...
    PJ["projects/과제/"]
    BF["봇 폴더 prodev/bots/prodev-과제-bot<br/>설정 두 장 settings.json · settings.local.json"]
...
이렇게 보이면 됨: 입력칸은 비어 있고 안내 글자 `봇에게 가지 않습니다 — 부르려면 @` 가 보이며 (`web/glue.js:7`), 방 머리의 봇 칩이 `⚪ prodev-수율개선-bot`(꺼짐)입니다. 오른쪽 조종석 판은 admin 에게 펼쳐져 있습니다. member 에게는 접혀 있어서 위쪽 `조종석` 단추로 폅니다.
...
### 걸음 18 — 첫 글을 보냅니다
입력칸에 `@` 를 칩니다. 자동완성 목록에 `TO prodev-수율개선-bot` 한 줄이 뜨면 Enter(또는 Tab · 누르기)로 고릅니다. 입력칸에 `@TO(prodev-수율개선-bot) ` 이 들어갑니다 (`web/app.js:708` · `:772`). 손으로 `@TO(prodev-수율개선-bot) ` 을 쳐도 같습니다. **그 뒤에** 아래 글을 붙입니다:
```
...
| 봉투 없음 | 봇에게 안 갑니다. 입력칸이 비었거나 봉투가 없으면 안내 글자 `봇에게 가지 않습니다 — 부르려면 @` 가 보입니다 (`web/glue.js:7` · `:34-35`) |
...
아래 입력 글에는 봉투까지 들어 있습니다. 입력칸은 방을 열 때도, 글을 보낸 뒤에도 비어 있으니 그대로 붙여 넣으세요. 봇은 켜져 있다고 가정합니다.
...
1) `김과제` 로 들어갑니다. 입력칸 왼쪽의 클립 단추(`파일 첨부`)로 아무 작은 파일(예 `yield.csv`)을 고르고, 아래 글과 함께 보냅니다:
```
어제 라인 3 수율 자료 올려요
```
2) 봉투 없이 한 줄 더 보냅니다:
...
```
`@CC` 는 `@` 자동완성에 없습니다(목록은 TO 한 줄뿐, `web/app.js:708`). 손으로 칩니다.
...
| `과제가 이미 있습니다: …` · `봇 폴더가 이미 있습니다: …` | 같은 이름이 있거나, 앞에서 실패한 봇 폴더가 `prodev/bots/` 에 남았습니다. 실패 때 못 지운 자리는 응답의 `left` 칸에 적힙니다 (`src/rooms/create.js:69-78`) | 다른 이름을 쓰거나, 남은 폴더를 PL 과 확인합니다 |
...
| 방에 `⛔ 시간 초과 거부 (10분) · <도구>` | 승인 카드에 admin 이 `approvalTimeoutMin`(기본 10분) 안에 답하지 않았습니다 | 봇에게 다시 시킵니다 |
