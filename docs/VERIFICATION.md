...
| `smoke/m2-compact.mjs` | M2 | 글 → `idle` 에서 세션 관리자 `compact()` → 글 (실증 5 의 말 셋) | `COMPACTED yes|no` · `COMPACT_BOUNDARY pre= trigger= {compact_metadata}` · `SYSTEM_MESSAGES <N>` · `HANDOFF 정상|못 썼다|없음` · `FIRST_TEXT_AFTER <앞 40자>` · `HOOKS […]` | 압축 · system 글 둘 · 인수인계서(`<봇 폴더>/handoff-compact.md`) · 압축 뒤 첫 답 |
