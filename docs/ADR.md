...
**덧붙임 (사람, 2026-09-15)** `@` 자동완성은 봇마다 `TO` 항목 하나만 낸다 — 옮긴 화면에서 **빼는** 것 하나다. 방마다 봇이 하나이고, 봉투 없는 글은 불렸을 때 `fetch_history` 로 따라잡으니(ADR-020) `CC` 를 고르는 항목이 쓸 데가 없다. 서버의 `@CC` 봉투 · 지시문 · 글 속 `@CC` 칩은 그대로다(손으로 치면 간다). 같은 날 위 결정의 "작성기의 `@TO(봇)` 미리 채움" 도 되돌렸다(ADR-018 상태) — cockpit 이 옮긴 화면에 더한 것은 접이식 판 하나다.
...
## ADR-018 봇에게 가는 글은 `@TO(봇)` · `@CC(봇)` 이 있는 글뿐 — ~~작성기가 `@TO(봇)` 을 미리 채운다~~ (미리 채움은 되돌림, 2026-09-15)
**상태** 확정(사람, R11 · 물음 ② "@TO 만", 후속 README 3.2 안 A). **미리 채움 부분만 되돌림 (사람, 2026-09-15)** — "대화창에 디폴트로 있는 @봇이름 은 지우고 … 사용성에 불필요해서이다". 입력칸은 방을 열 때도 · 보낸 뒤에도 비어 있다. 봇은 `@` 자동완성(항목 `TO` 하나, ADR-016 덧붙임)으로 부른다. 봉투 규칙 · 안내 글자 "봇에게 가지 않습니다 — 부르려면 @" 는 그대로다. 아래 결정 · 까닭의 미리 채움 문장은 옛 판의 기록이다.
