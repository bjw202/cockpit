...
## 2026-09-15 — `@` 자동완성에서 CC 항목 뺌 (사람 결정)

사람이 "`@` 를 누르면 봇마다 TO · CC 를 고르는데 필요 없다" 고 했다. 방마다 봇이 하나이고 작성기가 `@TO(봇)` 을 미리 채우며(ADR-018), 봉투 없는 글은 불렸을 때 `fetch_history` 로 따라잡는다(ADR-020). 목록 전체를 지우는 안과 TO 하나를 남기는 안 가운데 뒤엣것으로 정했다 — 봉투를 지운 뒤 긴 봇 이름을 손으로 치지 않게 하고, 안내 글자 "부르려면 @" 를 그대로 살린다.

**고친 것.**
- `web/app.js` `onComposerInput` — 항목 반복 `['TO', 'CC']` → `['TO']`. 이 함수는 이미 ARCHITECTURE 7.3 표의 고친 함수라 원본 지문 시험은 그대로 초록.
- `test/web-static.test.js` — `@ 자동완성 항목은 TO 하나 — CC 항목을 내지 않는다 (ADR-016 덧붙임)` 하나 더함.
- `docs/ADR.md` ADR-016 끝에 덧붙임 · `docs/ARCHITECTURE.md` 7.3 `onComposerInput` 줄 · `docs/PRD.md` F15.
