...
| F15 | **(v2) 화면은 minidiscord 화면을 옮긴 것이다** — 방 목록 사이드바 · 아바타 메시지 · 작성기 한 덩어리 · 계정 바 · 본문 마크다운 · `@TO` `@CC` 칩 · `@` 자동완성 · 붙여넣기 · 끌어놓기 · 썸네일 · UTC 표기 · 히스토리 끝까지 · SSE 백필. 옮기는 파일 다섯(`index.html` · `app.js` · `rich.js` · `style.css` · `design-tokens.css`)은 머리에 출처 커밋 · sha256. **디자인 토큰 34 의 값은 바꾸지 않는다.** 더하는 것은 오른쪽 접이식 판(F21)과 작성기의 `@TO(봇)` 미리 채움뿐 (R9 · ADR-016). ~~v1: 화면 셋: 채팅 판(방 둘 · 첨부 · 봇 상태 · 압축 경계) · 조종석 판 · 파일 판~~ — **대체됨 → ADR-016 · ADR-019** | 정적 시험: `web/design-tokens.css` 의 sha256 이 머리에 적힌 원본 sha256 과 같고 `--md-` 변수가 34 · `rich.js` 도 원본 sha256 과 같다 · 화면 순수 함수 시험(`web/glue.js`): 방을 열면 작성기 기본값이 `@TO(<그 방 봇 이름>) ` |
